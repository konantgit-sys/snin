"""SNIN First Contact — protocol for first appearance of an agent in the network.

When an agent appears, in ~50ms it:
  1. Scans 7 channels (mesh, nostr, tcp, api, mdns, gossip, route_engine)
  2. Builds routing matrix — who, where, how reachable
  3. Calculates agent ranks (T1-T4)
  4. Connects Nostr bridge
  5. Discovers IoT devices
  6. Builds architecture summary
"""
...
class FirstContact:
    """
    Full first contact cycle for an agent in the network.
    
    Used:
      - On agent start (agent_daemon → workflow)
      - On network failure (connectivity recovery)
      - On new hardware addition
    
    After completion — agent is ready:
      - knows all neighbors
      - knows channels and their latency
      - has routing matrix
      - knows its rank
    """
    ...
    async def scan_channels(self) -> dict:
        """Check all available communication channels."""
        ...
        # 5. Nostr (WebSocket, check via health)
    ...
    async def _check_nostr(self) -> dict:
        """Check Nostr bridge via health endpoint."""
        try:
    ...
    async def build_matrix(self) -> dict:
        """Build routing matrix from agents.json + mesh/stats."""
        nodes = {}
        edges = []
        
        # Read mesh/stats (no auth required)
        agent_count = 0
        try:
        ...
        # Read agents.json directly (bypass NIP-42 auth)
        agents_from_file = {}
        try:
    ...
    def rank_channels(self) -> list:
        """Sort channels by latency. Best first."""
        ranked = sorted(
    ...
    async def discover_iot(self) -> list:
        """Discover IoT devices via mDNS."""
        devices = []
        try:
            import zeroconf
            # stub — full implementation when hardware is connected
    ...
    async def scan_and_connect(self) -> dict:
        """
        Full first contact cycle.
        
        Returns:
            dict with results: scan_time_ms, total_time_ms, channels,
...
# ═══════════════════════════════════════════════════════════════
#  MATRIX SELF-LEARNING — Dynamic routing matrix
# ═══════════════════════════════════════════════════════════════

class MatrixUpdater:
    """
    Background thread for matrix exchange between agents.
    Starts after First Contact.
    """
    ...
    async def _ping_all(self) -> dict:
        ...
        async def _ping(pk, addr="127.0.0.1", port=9932):
            ...
            try:
                if _packet_privkey:
                    # ═══ Phase 1: signed kind:39000 instead of TCP connect ═══
    ...
    async def _exchange_matrices(self, ping_results: dict) -> list:
        """Exchange matrices with alive neighbors."""
        peer_matrices = []
    ...
    def _merge_matrices(self, peer_matrices: list, ping_results: dict) -> int:
        """Matrix merge: add new, update weights, cleanup dead."""
        changes = 0
        now = time.time()
...
# ═══════════════════════════════════════════════════════════════
#  DEVICE LAYER — Hardware connection
# ═══════════════════════════════════════════════════════════════
...
class DeviceConfig:
    """Device configuration in the SNIN network."""
...
class DeviceManager:
    """Device management: registration, storage."""
...
class DeviceLayer:
    """Device layer — FirstContact extension for hardware interaction."""
    ...
    async def flash_firmware(self, device_id: str) -> dict:
        ...
        flash_config = {
            ...
            "mesh_host": "mesh.example.com", "mesh_port": 9932,
